"""Native thinking Gemma 4 E4B full-source candidates, never automatic labels."""
...
VERSION = 'gemma4-e4b-thinking-target-candidate-v1'
...
def generate(record, *, tokenizer, student_tokenizer, model, model_digest,
    ...
    response = request_json(base + 'generate', {
        'model': model, 'prompt': prompt, 'raw': True, 'stream': False, 'think': True,
    ...
    return {'record_id': record['id'], 'source_diff_sha256': sha(record['diff']),
            ...
            'source_format': 'unified', 'output_format': 'json', 'thinking_enabled': True,
