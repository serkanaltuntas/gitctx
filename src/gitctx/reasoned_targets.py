"""Two-stage open-teacher comparison and commit generation with complete diffs.
...
VERSION = 'reasoned-target-candidate-v1'
COMPARISON_SYSTEM = (
    ...
    '300 words. For each changed area state what was true before and what is true after. '
...
def generate(record, *, tokenizer, student_tokenizer, model, model_digest,
    ...
    comparison_tokens = len(tokenizer.encode(comparison_prompt).ids)
    ...
    return {'record_id': record['id'], 'source_diff_sha256': sha(record['diff']),
            ...
            'comparison_is_verified': False, 'validation_errors': errors, 'training_approved': False,
