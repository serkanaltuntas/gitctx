"""Native Qwen3.5 structured change analysis Gemma 4 E4B full-source candidates, never automatic labels."""
...
VERSION = 'qwen35-9b-structured-change-target-v1'
SYSTEM = (
...
def generate(record, *, tokenizer, student_tokenizer, model, model_digest,
    ...
    if architecture != 'qwen35' or type(native_context) is not int or native_context < context:
        raise ValueError('native Qwen3.5 context capacity is unverified or insufficient')
    messages = [{'role': 'system', 'content': SYSTEM},
                {'role': 'user', 'content': prompt.split('<|im_start|>user\n', 1)[1].split('<|im_end|>', 1)[0]}]
    chat_response = request_json(base + 'chat', {
        ...
        'options': {'num_ctx': context, 'num_predict': 1024, 'temperature': temperature,
                    'seed': seed, 'num_thread': 4, 'top_p': 0.8, 'top_k': 20,
                    'presence_penalty': 1.5, 'repeat_penalty': 1.0, 'stop': ['<|im_end|>', '<|endoftext|>']}})
    ...
    return {'record_id': record['id'], 'source_diff_sha256': sha(record['diff']),
            ...
            'temperature': temperature, 'seed': seed, 'top_p': 0.8, 'top_k': 20,
            'presence_penalty': 1.5, 'repeat_penalty': 1.0, 'context_tokens': context,
