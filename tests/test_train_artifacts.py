...
class TrainArtifactTests(unittest.TestCase):
    def test_creates_and_validates_training_artifact_from_reviewed_labels(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(records[0]["label_source"], "teacher_generated_unverified_accepted")
            self.assertEqual(records[1]["label_source"], "teacher_generated_unverified_edited")
            self.assertIn("plain commit message", records[0]["messages"][0]["content"])
            self.assertFalse(records[0]["review_provenance"]["independent_factuality_review"])
