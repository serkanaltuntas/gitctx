...
from gitctx import gemma_reasoned_targets as rt
from test_student_input import record


class GemmaReasonedTargetsTests(TestCase):
    def setUp(self):
        ...
        self.calls = []
        self.comparison = 'The annotation changes; runtime is unchanged. <turn|>ignored'
    ...
    def api(self, url, payload=None):
        ...
        if url.endswith('show'):
            return {'model_info': {'general.architecture': 'gemma4', 'gemma4.context_length': 32768}}
    ...
    def test_capacity_is_checked_for_comparison_and_larger_final_prompt(self):
        def api(url, payload=None):
            response = self.api(url, payload)
            if url.endswith('show'):
                response['model_info']['gemma4.context_length'] = 4096
