...
from gitctx import gemma_chat_targets as gt
from test_student_input import record


class GemmaChatTargetsTests(TestCase):
    def setUp(self):
    ...
    def api(self, url, payload=None):
        ...
        return {'message': {'content': 'refactor(a): change value', 'thinking': 'unverified thought'},
    ...
    def test_native_roles_and_complete_payload_roundtrip_without_label_or_control_injection(self):
        ...
        self.assertTrue(self.calls[-1][1]['think'])
        self.assertNotIn('format', self.calls[-1][1])
        self.assertIn('<|think|>\n', prompt)
        self.assertEqual(self.calls[-1][1]['options']['num_predict'], 2048)
